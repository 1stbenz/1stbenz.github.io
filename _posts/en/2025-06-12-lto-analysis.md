...
faq:
  ...
  - question: "Why is LTO's continuous discharge rate lower than LiFePO4 despite having much lower internal resistance?"
    answer: "Mainly due to cathode 'concentration polarization' and an 'extremely narrow voltage window.' LTO has a nominal voltage of only 2.3V and a cut-off voltage of 1.5V, with a usable voltage drop of only ~0.8V (far smaller than LiFePO4's 1.2V). Under heavy current, polarization and voltage drop immediately plunge the cell below 1.5V, triggering cut-off shutdown even when temperatures remain normal."
...
Why is LTO's continuous discharge rate lower than LiFePO4 (20C vs. 50C–80C) despite having much lower internal resistance? The key reasons are:
* **Cathode Concentration Polarization**: The ultra-low internal resistance on spec sheets is only static or instantaneous impedance. During continuous high-current discharge, lithium-ion diffusion in the cathode cannot keep up (concentration polarization), causing voltage to drop rapidly.
* **Extremely Narrow Voltage Window**: LTO has a nominal voltage of only 2.3V (cut-off voltage 1.5V), offering a usable voltage drop window of only ~0.8V (far smaller than LiFePO4's 1.2V). Any slight voltage drop and polarization under heavy load instantly trips the 1.5V cut-off threshold, causing the system to shut off for low voltage.
