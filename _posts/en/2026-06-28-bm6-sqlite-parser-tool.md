...
faq:
  - question: "How is this parser different from the Monthly Log File Analyzer?"
    answer: "This tool directly reads the complete SQLite database file stored locally by the BM6 APP on your phone. In addition to analyzing monthly historical voltage and driving trips, it can also parse high-frequency (100Hz) engine cranking test voltage waveforms (Cranking Waveform) and charging/ripple test records."
  - question: "How do I get the BM6.sqlite file?"
    answer: "On iPhone or Android phones, you can use file sharing or backup software (such as iTunes, iMazing, Android File Transfer, etc.) to access the BM6 APP's sandbox directory (Documents), where you can find the database file named BM6.sqlite or BM6.db."
...
<div id="bm6-sqlite-app-container">
    ...
    <h2 id="ui-title" style="text-align: center; color: #c9d1d9; margin-bottom: 10px; margin-top: 10px;">BM6 Battery Test Database (SQLite) Parser</h2>
    ...
    <div class="upload-section" style="margin-bottom: 30px; padding: 20px; background: #161b22; border-radius: 8px; border: 2px dashed #1f6feb; text-align: center;">
        <p>Please select your <b>BM6.sqlite</b> file (data is processed entirely on your computer, not uploaded):</p>
    ...
    <div id="filter-section" style="display: none; background: #161b22; padding: 20px; border-radius: 8px; margin-bottom: 30px; border: 1px solid #30363d; text-align: center;">
        <div style="display: flex; gap: 20px; flex-wrap: wrap; align-items: center; justify-content: center;">
            <div style="display: flex; align-items: center; gap: 8px;">
                <label style="font-weight: bold; color: #c9d1d9;">Select Device (MAC):</label>
            ...
            <div style="display: flex; align-items: center; gap: 8px;">
                <label style="font-weight: bold; color: #c9d1d9;">Select Month:</label>
    ...
    <div id="history-section" class="section">
        <h2>📈 Historical Monthly Data and Trip Analysis</h2>
        <div style="background: rgba(210, 153, 34, 0.1); padding: 15px 20px; border-radius: 10px; margin-bottom: 25px; border-left: 5px solid #d29922; font-size: 14.5px; line-height: 1.6; color: #8b949e;">
            <strong style="color: #d29922;">Historical Record Trip Analysis</strong><br>
            <span>Automatically analyzes historical voltage and temperature fluctuations, identifies driving trips, and calculates alternator and battery health. Supports scroll/pinch zoom and drag pan.</span>
        </div>
        ...
        <div style="background: #161b22; border: 1px solid #30363d; padding: 15px 25px; border-radius: 10px; margin-bottom: 25px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 15px;">
            <div style="display: flex; gap: 10px; align-items: center;">
                <label style="font-weight: bold; color: #c9d1d9; font-size: 14px;">Display Range:</label>
        ...
        <div id="stats-container" class="stats-grid">
            ...
            <div class="stat-card" style="background: rgba(137, 87, 229, 0.1); border: 1px solid rgba(137, 87, 229, 0.3);">
                <div style="font-size: 13px; color: #a371f7; margin-bottom: 5px; font-weight: 500;">Avg. Driving Voltage (Alternator)</div>
        ...
        <div style="background: #161b22; padding: 15px; border-radius: 10px; border: 1px solid #30363d; margin-bottom: 25px; display: flex; flex-direction: column; gap: 20px;">
            <div>
                <div class="chart-header-action">
                    <span style="font-size: 14px; font-weight: bold; color: #58a6ff;">⚡ Historical Voltage Trend</span>
                    <button class="chart-save-btn" onclick="saveChartWithWatermark('voltageChart', 'bm6_history_voltage.png', 'BM6 Historical Voltage Trend')">
                        📷 Save Screenshot
                    </button>
                </div>
            ...
            <div>
                <div class="chart-header-action">
                    <span style="font-size: 14px; font-weight: bold; color: #f85149;">🌡️ Historical Temperature Trend</span>
                    <button class="chart-save-btn" onclick="saveChartWithWatermark('tempChart', 'bm6_history_temperature.png', 'BM6 Historical Temperature Trend')">
                        📷 Save Screenshot
                    </button>
                </div>
                <div style="height: 280px; width: 100%;">
    ...
    <div id="charging-section" class="section">
        <h2>⚡ Charging and Ripple Test Records</h2>
        <p>This table records data including idle/high-speed charging voltage, diode ripple voltage, and more.</p>
        <table id="charging-table">
            <thead>
                <tr>
                    <th style="width: 25%;">Device & Time Information</th>
                    <th style="width: 75%;">Key Data Parsed (Synchronized with APP)</th>
                </tr>
            </thead>
            <tbody></tbody>
        </table>
    ...
    <div id="cranking-section" class="section">
        <h2>🚗 Cranking Test Waveforms</h2>
        <p>This table records high-frequency voltage change values, automatically plotted for you.</p>
        <div id="cranking-stats-container" class="stats-grid-2">
            <div class="stat-card" style="background: rgba(31, 111, 235, 0.1); border: 1px solid rgba(31, 111, 235, 0.3);">
                <div style="font-size: 13px; color: #58a6ff; margin-bottom: 5px; font-weight: 500;">Average Cranking Time</div>
            ...
            <div class="stat-card" style="background: rgba(227, 76, 38, 0.1); border: 1px solid rgba(227, 76, 38, 0.3);">
                <div style="font-size: 13px; color: #f0883e; margin-bottom: 5px; font-weight: 500;">Average Cranking Voltage</div>
...
<script>
    ...
    document.getElementById('dbfile').addEventListener('change', function(event) {
        ...
        reader.onload = function() {
            if(!SQL) {
                alert("SQL.js is still loading, please wait a second and try again.");
                return;
            }
            ...
            try {
                ...
                alert("Failed to read database, please confirm the file is correct. Error message: " + err.message);
            }
        };
        reader.readAsArrayBuffer(file);
    });

    // Lightweight Apple Binary Plist (bplist00) and NSKeyedArchiver Parser
    ...
    function parseChargingTable(db, serial, month) {
        ...
        try {
            ...
            if (res.length === 0) {
                tbody.innerHTML = '<tr><td colspan="2">No charging test records for this device/month.</td></tr>';
                return;
            }
            ...
            values.forEach(row => {
                ...
                const mac = row[macIndex] || "Unknown Device";
                ...
                let rippleVolStr = "No data";
                let loadVolStr = "No data";
                let noLoadVolStr = "No data";
                ...
                if (blob) {
                    try {
                        ...
                        if (parsedData) {
                            ...
                            loadVolStr = parsedData.loadVol !== undefined ? `${Number(parsedData.loadVol).toFixed(2)} V` : "No data";
                            
                            // 3. Handle Idle Charging Voltage (NoLoad)
                            noLoadVolStr = parsedData.noLoadVol !== undefined ? `${Number(parsedData.noLoadVol).toFixed(2)} V` : "No data";
                            
                            parseSuccess = true;
                        }
                    } catch (err) {
                        console.error("Failed to parse bplist:", err);
                    }
                }
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <b>Idle Charging Voltage (NoLoad):</b> <span style="color:#58a6ff; font-weight:bold;">${noLoadVolStr}</span> <br>
                        <b>High-Speed Charging Voltage (Load):</b> <span style="color:#3fb950; font-weight:bold;">${loadVolStr}</span> <br>
                        <b>Diode Ripple Voltage (Ripple):</b> <span style="color:#d2a8ff; font-weight:bold;">${rippleVolStr}</span> <br>
                        <small style="color:gray;">Parsing Status: ${parseSuccess ? "🟢 Successfully parsed" : "🔴 Parsing failed"}</small>
                    </td>
                `;
                tbody.appendChild(tr);
            });
        } catch (e) {
            tbody.innerHTML = `<tr><td colspan="2">Parsing error: ${e.message}</td></tr>`;
        }
    }
    ...
    // Handle tbl_cranking (Cranking Waveform)
    function parseCrankingTable(db, serial, month) {
        ...
        try {
            ...
            const res = db.exec(sql);
            if (res.length === 0) {
                container.innerHTML = '<p>No cranking waveform records for this device/month.</p>';
            ...
            values.forEach((row, index) => {
                ...
                if (extractedNumbers.length > 0) {
                    ...
                    // Calculate cranking time and accumulate stats (calculated up to starter disengagement/orange point)
                    let startIdx = minIdx;
                    ...
                    if (serialIndex !== -1 && row[serialIndex] && ts) {
                        ...
                        try {
                            ...
                            console.error("Failed to query historical temperature:", tempErr);
                        }
                    }

                    let titleText = `Cranking Time: ${crankingDuration.toFixed(2)}s - `;
                    if (dateStr) {
                        titleText += ` (Time: ${dateStr})`;
                    } else if (dateIndex !== -1 && row[dateIndex]) {
                        titleText += ` Time: ${row[dateIndex]}`;
                    }
                    if (eventTemp !== null && eventTemp !== undefined) {
                        titleText += ` (Temperature: ${eventTemp}°C)`;
                    } else if (tempIndex !== -1 && row[tempIndex] !== undefined) {
                        titleText += ` (Temperature: ${row[tempIndex]}°C)`;
                    }
                    ...
                    saveBtn.title = 'Download chart screenshot with watermark';
                    ...
                    wrapper.innerHTML = `<h3>Test Record #${row[0] || index + 1}</h3><p style="color:red; margin: 15px 0;">Unable to parse continuous voltage values from this record.</p><hr>`;
            ...
            container.innerHTML = `<p>Failed to load: ${e.message}</p>`;
            document.getElementById('cranking-stats-container').style.display = 'none';
        }
    }
    ...
    function parseHistoryTable(db, serial, month) {
        ...
        try {
            ...
            if (res.length === 0) {
                ...
                document.getElementById('dateSelector').innerHTML = '<option value="all">-- No Date --</option>';
                return;
            }
    ...
    function renderTrips(result) {
        ...
        const modeSuffix = result.mode === 'LiFePO4' ? ' [LiFePO4 Mode]' : ' [Lead-Acid / Charging Control Mode]';
        ...
        result.trips.forEach((t, i) => {
            ...
            rowDiv.innerHTML = `<span style="font-size: 14px; color: #c9d1d9;"><b style="color:#58a6ff;">Trip ${i+1}</b>：${t.startStr} ~ ${t.endStr} (Duration ${t.duration} minutes)</span>`;

            const btn = document.createElement('button');
            btn.innerHTML = '🔍 Zoom to Chart';
    ...
    function initDeviceSelector(db) {
        ...
        if (serials.size === 0) {
            alert("No device MAC data found in the database!");
            return;
        }
    ...
    function exportVisibleHistoryCsv() {
        ...
        if (visibleData.length === 0) {
            alert("No data within the current visible display range.");
            return;
        }
    ...
    function saveChartWithWatermark(chartOrCanvas, defaultFileName, chartTitle) {
        ...
        if (!canvas || !(canvas instanceof HTMLCanvasElement)) {
            alert("Chart not found for screenshot.");
            return;
        }
        ...
        // 1. Draw dark background (matching site theme)
        ctx.fillStyle = '#0d1117';
        ctx.fillRect(0, 0, exportCanvas.width, exportCanvas.height);
        
        // 2. Draw top title bar (if title provided)
        if (chartTitle) {
        ...
        // 3. Draw chart content
        ctx.drawImage(canvas, 0, topPadding);
        
        // 4. Draw bottom watermark footer
        ...
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ctx.fillText('BM6 Battery Monitor', Math.round(16 * ratio), footerY + watermarkHeight / 2);
