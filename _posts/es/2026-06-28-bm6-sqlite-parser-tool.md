...
faq:
  - question: "¿En qué se diferencia este analizador de la herramienta de análisis de registros mensuales?"
    answer: "Esta herramienta lee directamente el archivo completo de la base de datos SQLite que la aplicación BM6 guarda localmente en el teléfono. Además de analizar el voltaje histórico mensual y los viajes, también puede analizar la forma de onda de voltaje de la prueba de arranque del motor (Cranking Waveform) de alta frecuencia (100Hz) y los registros de detección de carga/ondulación."
  - question: "¿Cómo obtengo el archivo BM6.sqlite?"
    answer: "En un teléfono iPhone o Android, puede acceder al directorio sandbox de la aplicación BM6 (Documents) a través de software para compartir archivos o de copia de seguridad (como iTunes, iMazing, Android File Transfer, etc.) y encontrar el archivo de base de datos llamado BM6.sqlite o BM6.db."
...
<div id="bm6-sqlite-app-container">
    ...
    <h2 id="ui-title" style="text-align: center; color: #c9d1d9; margin-bottom: 10px; margin-top: 10px;">Analizador de Base de Datos (SQLite) de Pruebas de Batería BM6</h2>
    ...
    <div class="upload-section" style="margin-bottom: 30px; padding: 20px; background: #161b22; border-radius: 8px; border: 2px dashed #1f6feb; text-align: center;">
        <p>Por favor, seleccione su archivo <b>BM6.sqlite</b> (los datos se procesan completamente en su computadora, no se suben):</p>
    ...
    <div id="filter-section" style="display: none; background: #161b22; padding: 20px; border-radius: 8px; margin-bottom: 30px; border: 1px solid #30363d; text-align: center;">
        <div style="display: flex; gap: 20px; flex-wrap: wrap; align-items: center; justify-content: center;">
            <div style="display: flex; align-items: center; gap: 8px;">
                <label style="font-weight: bold; color: #c9d1d9;">Seleccionar Dispositivo (MAC):</label>
            ...
            <div style="display: flex; align-items: center; gap: 8px;">
                <label style="font-weight: bold; color: #c9d1d9;">Seleccionar Mes:</label>
    ...
    <div id="history-section" class="section">
        <h2>📈 Datos Mensuales Históricos y Análisis de Viajes</h2>
        <div style="background: rgba(210, 153, 34, 0.1); padding: 15px 20px; border-radius: 10px; margin-bottom: 25px; border-left: 5px solid #d29922; font-size: 14.5px; line-height: 1.6; color: #8b949e;">
            <strong style="color: #d29922;">Análisis de Viajes del Historial</strong><br>
            <span>Analiza automáticamente las fluctuaciones históricas de voltaje y temperatura, identifica los viajes y calcula la salud del alternador y la batería. Soporta zoom con rueda/dos dedos y arrastre para desplazar.</span>
        </div>
        ...
        <div style="background: #161b22; border: 1px solid #30363d; padding: 15px 25px; border-radius: 10px; margin-bottom: 25px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 15px;">
            <div style="display: flex; gap: 10px; align-items: center;">
                <label style="font-weight: bold; color: #c9d1d9; font-size: 14px;">Rango de Visualización:</label>
            ...
            <button id="exportCsvBtn" style="padding: 8px 16px; border-radius: 8px; border: 1px solid #30363d; font-size: 14px; background: #21262d; color: #58a6ff; font-weight: bold; cursor: pointer; transition: 0.2s; white-space: nowrap;">
                📥 Exportar rango actual CSV
            </button>
        </div>

        <div id="stats-container" class="stats-grid">
            <div class="stat-card" style="background: rgba(31, 111, 235, 0.1); border: 1px solid rgba(31, 111, 235, 0.3);">
                <div style="font-size: 13px; color: #58a6ff; margin-bottom: 5px; font-weight: 500;">Número Total de Viajes</div>
            ...
            <div class="stat-card" style="background: rgba(35, 134, 54, 0.1); border: 1px solid rgba(35, 134, 54, 0.3);">
                <div style="font-size: 13px; color: #3fb950; margin-bottom: 5px; font-weight: 500;">Tiempo Total de Conducción</div>
            ...
            <div class="stat-card" style="background: rgba(227, 76, 38, 0.1); border: 1px solid rgba(227, 76, 38, 0.3);">
                <div style="font-size: 13px; color: #f0883e; margin-bottom: 5px; font-weight: 500;">Voltaje Mínimo (Arranque en Frío)</div>
            ...
            <div class="stat-card" style="background: rgba(137, 87, 229, 0.1); border: 1px solid rgba(137, 87, 229, 0.3);">
                <div style="font-size: 13px; color: #a371f7; margin-bottom: 5px; font-weight: 500;">Voltaje Promedio de Conducción (Alternador)</div>
        ...
        <div style="background: #161b22; padding: 15px; border-radius: 10px; border: 1px solid #30363d; margin-bottom: 25px; display: flex; flex-direction: column; gap: 20px;">
            <div>
                <div class="chart-header-action">
                    <span style="font-size: 14px; font-weight: bold; color: #58a6ff;">⚡ Tendencia histórica de voltaje (Voltage)</span>
                    <button class="chart-save-btn" onclick="saveChartWithWatermark('voltageChart', 'bm6_history_voltage.png', 'BM6 Tendencia histórica de voltaje')">
                        📷 Guardar captura
                    </button>
                </div>
            ...
            <div>
                <div class="chart-header-action">
                    <span style="font-size: 14px; font-weight: bold; color: #f85149;">🌡️ Tendencia histórica de temperatura (Temperature)</span>
                    <button class="chart-save-btn" onclick="saveChartWithWatermark('tempChart', 'bm6_history_temperature.png', 'BM6 Tendencia histórica de temperatura')">
                        📷 Guardar captura
                    </button>
                </div>
                <div style="height: 280px; width: 100%;">
        ...
        <div id="trip-section" style="margin-top: 20px; padding: 15px; background: #161b22; border-radius: 10px; border: 1px dashed #30363d; display: none; margin-bottom: 30px;">
            <h3 id="ui-trip-title" style="color: #c9d1d9; font-size: 16px; margin-top: 0; margin-bottom: 15px;">Viajes Detectados</h3>
            <div id="trip-list" style="display: flex; flex-direction: column; gap: 8px;"></div>
        </div>
    </div>

    <div id="charging-section" class="section">
        <h2>⚡ Registro de Pruebas de Carga y Ondulación</h2>
        <p>Esta tabla registra datos como el voltaje de carga en ralentí/alta velocidad y el voltaje de ondulación del diodo.</p>
        <table id="charging-table">
            <thead>
                <tr>
                    <th style="width: 25%;">Información de Dispositivo y Hora</th>
                    <th style="width: 75%;">Datos Clave Analizados (Sincronizados con la APP)</th>
                </tr>
            </thead>
            <tbody></tbody>
        </table>
    ...
    <div id="cranking-section" class="section">
        <h2>🚗 Forma de Onda de Prueba de Arranque</h2>
        <p>Esta tabla registra valores de variación de voltaje de alta frecuencia, los cuales se han graficado automáticamente para usted.</p>
        <div id="cranking-stats-container" class="stats-grid-2">
            <div class="stat-card" style="background: rgba(31, 111, 235, 0.1); border: 1px solid rgba(31, 111, 235, 0.3);">
                <div style="font-size: 13px; color: #58a6ff; margin-bottom: 5px; font-weight: 500;">Tiempo promedio de arranque</div>
                <div id="stat-avg-crank-time" style="font-size: 24px; font-weight: bold; color: #79c0ff;">0.00 <span style="font-size: 12px; font-weight: normal;">s</span></div>
            </div>
            <div class="stat-card" style="background: rgba(227, 76, 38, 0.1); border: 1px solid rgba(227, 76, 38, 0.3);">
                <div style="font-size: 13px; color: #f0883e; margin-bottom: 5px; font-weight: 500;">Voltaje promedio de arranque</div>
...
<script>
    ...
    document.getElementById('dbfile').addEventListener('change', function(event) {
        ...
        reader.onload = function() {
            if(!SQL) {
                alert("SQL.js aún se está cargando, por favor, espere un segundo e intente de nuevo.");
                return;
            }
            ...
            try {
                ...
                alert("Error al leer la base de datos, por favor, confirme que el archivo es correcto. Mensaje de error：" + err.message);
            }
        };
        reader.readAsArrayBuffer(file);
    });

    // Analizador ligero de Apple Binary Plist (bplist00) y NSKeyedArchiver
    ...
    // Procesar tbl_charging (ondulación y carga)
    function parseChargingTable(db, serial, month) {
        ...
        try {
            // En tbl_charging, el nombre de la columna es 'serial' en lugar de 'mac'
            ...
            if (res.length === 0) {
                tbody.innerHTML = '<tr><td colspan="2">No hay registros de prueba de carga para este dispositivo/mes.</td></tr>';
                return;
            }
            ...
            values.forEach(row => {
                ...
                const mac = row[macIndex] || "Dispositivo desconocido";
                
                let date = "Hora desconocida";
                ...
                let rippleVolStr = "Sin datos";
                let loadVolStr = "Sin datos";
                let noLoadVolStr = "Sin datos";
                ...
                if (blob) {
                    try {
                        ...
                        if (parsedData) {
                            // 1. Procesar voltaje de ondulación del diodo (Ripple) y conversión de unidades (V -> mV)
                            if (parsedData.rippleVol !== undefined) {
                                let rVal = Number(parsedData.rippleVol);
                                // Si los datos son menores que 1 (ej. 0.001 V), convertir a milivoltios (1 mV)
                                if (rVal > 0 && rVal < 1) {
                                    rVal = rVal * 1000;
                                }
                            ...
                            // 2. Procesar voltaje de carga a alta velocidad (Load)
                            loadVolStr = parsedData.loadVol !== undefined ? `${Number(parsedData.loadVol).toFixed(2)} V` : "Sin datos";
                            
                            // 3. Procesar voltaje de carga en ralentí (NoLoad)
                            noLoadVolStr = parsedData.noLoadVol !== undefined ? `${Number(parsedData.noLoadVol).toFixed(2)} V` : "Sin datos";
                            
                            parseSuccess = true;
                        }
                    } catch (err) {
                        console.error("Error al analizar bplist:", err);
                    }
                }
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <b>Voltaje de Carga en Ralentí (Sin Carga):</b> <span style="color:#58a6ff; font-weight:bold;">${noLoadVolStr}</span> <br>
                        <b>Voltaje de Carga a Alta Velocidad (Con Carga):</b> <span style="color:#3fb950; font-weight:bold;">${loadVolStr}</span> <br>
                        <b>Voltaje de Ondulación del Diodo (Ripple):</b> <span style="color:#d2a8ff; font-weight:bold;">${rippleVolStr}</span> <br>
                        <small style="color:gray;">Estado del análisis: ${parseSuccess ? "🟢 Análisis exitoso y preciso" : "🔴 Análisis fallido"}</small>
                    </td>
                `;
                tbody.appendChild(tr);
            });
        } catch (e) {
            tbody.innerHTML = `<tr><td colspan="2">Error de análisis: ${e.message}</td></tr>`;
        }
    }
    ...
    // Procesar tbl_cranking (forma de onda de arranque)
    function parseCrankingTable(db, serial, month) {
        ...
        // Configurar valores predeterminados de Chart
        ...
        try {
            ...
            const res = db.exec(sql);
            if (res.length === 0) {
                container.innerHTML = '<p>No hay registros de forma de onda de arranque para este dispositivo/mes.</p>';
            ...
            values.forEach((row, index) => {
                ...
                if (extractedNumbers.length > 0) {
                    ...
                    // 3. Punto de desconexión del motor de arranque (Starter Disengaged)
                    ...
                    // Calcular el tiempo de arranque y acumular estadísticas (calculado hasta la desconexión del motor de arranque/punto naranja)
                    let startIdx = minIdx;
                    ...
                    // Plugin para dibujar etiquetas de puntos
                    ...
                    if (serialIndex !== -1 && row[serialIndex] && ts) {
                        ...
                        try {
                            ...
                            console.error("Error al consultar la temperatura histórica:", tempErr);
                        }
                    }

                    let titleText = `Tiempo de arranque: ${crankingDuration.toFixed(2)}s - `;
                    if (dateStr) {
                        titleText += ` (Hora: ${dateStr})`;
                    } else if (dateIndex !== -1 && row[dateIndex]) {
                        titleText += ` Hora: ${row[dateIndex]}`;
                    }
                    if (eventTemp !== null && eventTemp !== undefined) {
                        titleText += ` (Temperatura: ${eventTemp}°C)`;
                    } else if (tempIndex !== -1 && row[tempIndex] !== undefined) {
                        titleText += ` (Temperatura: ${row[tempIndex]}°C)`;
                    }
                    titleText += ` (Inicial: ${vInitial.toFixed(2)}V)`;
                    ...
                    saveBtn.innerHTML = '📷 Guardar captura';
                    saveBtn.title = 'Descargar captura de gráfico con marca de agua';
                                ...
                                tooltip: {
                                    ...
                                    callbacks: {
                                        ...
                                        label: function(context) {
                                            ...
                                            return `Tiempo: ${context.raw.x.toFixed(2)} Segundos`;
                                        }
                                    }
                                },
                                zoom: {
                    ...
                    wrapper.innerHTML = `<h3>Registro de prueba #${row[0] || index + 1}</h3><p style="color:red; margin: 15px 0;">No se pudieron analizar valores de voltaje continuos de este registro.</p><hr>`;
            ...
            if (validCrankCount > 0) {
                ...
                document.getElementById('stat-avg-crank-time').innerHTML = `${avgCrankTime} <span style="font-size:12px; font-weight:normal;">s</span>`;
            ...
            container.innerHTML = `<p>Error de lectura: ${e.message}</p>`;
            document.getElementById('cranking-stats-container').style.display = 'none';
        }
    }
    ...
    function initHistoryCharts() {
        ...
        vChart = new Chart(document.getElementById('voltageChart'), createChartConfig('Voltaje (V)', '#58a6ff', 'Voltaje (V)'));
        tChart = new Chart(document.getElementById('tempChart'), createChartConfig('Temperatura (℃)', '#f85149', 'Temperatura (℃)'));
    }
    ...
    function parseHistoryTable(db, serial, month) {
        ...
        try {
            ...
            if (res.length === 0) {
                ...
                document.getElementById('dateSelector').innerHTML = '<option value="all">-- Sin fecha --</option>';
                return;
            }
            ...
            dateSelector.innerHTML = '<option value="all">-- Mostrar Todo --</option>';
    ...
    function renderTrips(result) {
        ...
        const modeSuffix = result.mode === 'LiFePO4' ? ' [Modo LiFePO4]' : ' [Modo Plomo-Ácido / Control de Carga]';
        document.getElementById('ui-trip-title').innerText = 'Viajes Detectados' + modeSuffix;
        ...
        result.trips.forEach((t, i) => {
            ...
            rowDiv.innerHTML = `<span style="font-size: 14px; color: #c9d1d9;"><b style="color:#58a6ff;">Viaje ${i+1}</b>：${t.startStr} ~ ${t.endStr} (Duración ${t.duration} minutos)</span>`;

            const btn = document.createElement('button');
            btn.innerHTML = '🔍 Zoom del Gráfico';
    ...
    function initDeviceSelector(db) {
        ...
        if (serials.size === 0) {
            alert("¡No se encontraron datos de MAC de dispositivo en la base de datos!");
            return;
        }
    ...
    function initMonthSelector(db, serial) {
        ...
        monthSelector.innerHTML = '<option value="all">-- Todos los meses --</option>';
    ...
    function exportVisibleHistoryCsv() {
        ...
        let minX = (vChart && vChart.scales && vChart.scales.x) ? vChart.scales.x.min : null;
        let maxX = (vChart && vChart.scales && vChart.scales.x) ? vChart.scales.x.max : null;

        if (minX === null || minX === undefined) minX = -Infinity;
        if (maxX === null || maxX === undefined) maxX = Infinity;

        const visibleData = allData.filter(d => d.x >= minX && d.x <= maxX);

        if (visibleData.length === 0) {
            alert("No hay datos dentro del rango de visualización actual.");
            return;
        }
        ...
        visibleData.forEach(d => {
            ...
            const min = String(dateObj.getMinutes()).padStart(2, '0');
            const ss = String(dateObj.getSeconds()).padStart(2, '0');
            const fullTimeStr = `${yyyy}-${mm}-${dd} ${hh}:${min}:${ss}`;
            csvContent += `"${fullTimeStr}",${d.v}\n`;
        });
        ...
        const filename = `bm6_voltage_${selectedDevice || 'export'}_${new Date().toISOString().slice(0,10)}.csv`;
    ...
    function saveChartWithWatermark(chartOrCanvas, defaultFileName, chartTitle) {
        ...
        if (!canvas || !(canvas instanceof HTMLCanvasElement)) {
            alert("No se encontró el gráfico para la captura.");
            return;
        }
        ...
        // 1. Dibujar fondo oscuro (coincide con el tema)
        ctx.fillStyle = '#0d1117';
        ctx.fillRect(0, 0, exportCanvas.width, exportCanvas.height);
        
        // 2. Dibujar barra de título superior (si se proporciona)
        if (chartTitle) {
        ...
        // 3. Dibujar contenido del gráfico
        ctx.drawImage(canvas, 0, topPadding);
        
        // 4. Dibujar pie de página con marca de agua
        ...
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ctx.fillText('BM6 Monitor de Batería', Math.round(16 * ratio), footerY + watermarkHeight / 2);
        
        // Pie de página derecho (URL de marca de agua)
        ...
        try {
            ...
            alert("Error al guardar la captura: " + err.message);
        }
    }
