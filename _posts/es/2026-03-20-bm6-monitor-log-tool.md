---
layout: post
title: "Herramienta de análisis para archivos mensuales BM6/BM200"
lang: es
...
faq:
  - question: "¿Qué problema resuelve principalmente esta herramienta?"
    answer: "La aplicación original BM6/BM200 a menudo no puede determinar con precisión si el motor ha parado cuando la batería de litio hierro tiene una tensión en reposo alta, lo que resulta en registros de recorrido anómalos o interrumpidos. Esta herramienta utiliza un algoritmo inteligente optimizado específicamente para filtrar las fluctuaciones de voltaje y restaurar cada viaje real."
  - question: "¿Es aplicable a las baterías de plomo-ácido convencionales o los vehículos equipados con «controlador de carga (alternador inteligente)»?"
    answer: "Totalmente aplicable. El algoritmo del sistema analiza automáticamente la tensión base, determina si la batería actual está en «modo hierro-litio» o «modo plomo-ácido / controlador de carga», y ajusta dinámicamente la lógica de decisión para capturar con precisión el recorrido."
  - question: "¿Cómo puedo obtener y subir los datos?"
    answer: "Por favor, primero exporte el historial en la aplicación BM6/BM200 del dispositivo móvil como un archivo Excel (.xls o .xlsx), luego suba ese archivo a esta página de herramientas para que se realice automáticamente el análisis."
  - question: "¿Qué datos de análisis proporciona esta herramienta?"
    answer: "La herramienta calculará automáticamente para usted: el número total de viajes, la duración total del viaje, la tensión mínima (referencia para arranque en frío) y la tensión media durante el viaje (saludabilidad del generador), además de proporcionar gráficos de líneas visuales escalables con libertad para voltaje y temperatura."
---
...
<div id="car-app-container" style="max-width: 950px; margin: 20px auto; font-family: 'Segoe UI', 'Microsoft JhengHei', sans-serif; background: #0d1117; color: #c9d1d9; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); border: 1px solid #30363d; position: relative;">
    ...
    <h2 id="ui-title" style="text-align: center; color: #c9d1d9; margin-bottom: 10px; margin-top: 10px;">Herramienta de análisis de registros BM6/BM200</h2>
    ...
    <div style="background: rgba(210, 153, 34, 0.1); padding: 15px 20px; border-radius: 10px; margin-bottom: 25px; border-left: 5px solid #d29922; font-size: 14.5px; line-height: 1.6; color: #8b949e;">
        <strong id="ui-desc-title" style="color: #d29922;">¿Por qué se necesita esta herramienta?</strong><br>
        <span id="ui-desc-body">Aunque BM6/BM200 soporta el registro continuo después de apagar el motor, la APP no determina con precisión los recorridos y es difícil observar los detalles de las ondas. **Esta herramienta corrige ese problema** mediante un método de análisis diseñado por nosotros mismos, permitiendo analizar con precisión las fluctuaciones de voltaje, reconstruir cada recorrido y proporcionar una visualización intuitiva del voltaje y la temperatura.</span>
    </div>
...
<div style="display: flex; gap: 10px;">
            <button id="uploadTrigger" style="background: #1f6feb; color: white; border: none; padding: 10px 20px; border-radius: 8px; cursor: pointer; font-weight: bold; font-size: 14px;">📁 Subir archivo Excel</button>
            
            <button id="loadSampleBtn" style="background: #30363d; color: #c9d1d9; border: 1px solid #8b949e; padding: 9px 20px; border-radius: 8px; cursor: pointer; font-weight: bold; font-size: 14px; display: flex; align-items: center; transition: 0.2s;">📄 Cargar directamente un archivo de ejemplo</button>            
     </div>
        <div style="display: flex; gap: 10px; align-items: center;">
            <label id="ui-display-range" style="font-weight: bold; color: #c9d1d9; font-size: 14px;">Rango de visualización:</label>
            <select id="dateSelector" style="padding: 8px 12px; border-radius: 8px; border: 1px solid #30363d; font-size: 14px; min-width: 160px; cursor: pointer; background: #0d1117; color: #c9d1d9;">
                <option value="all" id="ui-show-all">-- Mostrar todo --</option>
            </select>
        </div>
        <input type="file" id="fileInput" accept=".xls,.xlsx" style="display:none">
    </div>

<div id="status-text" style="text-align: center; color: #8b949e; margin-bottom: 20px; font-size: 14px;">Suba el archivo Excel exportado de BM6/BM200, o descargue primero un archivo de ejemplo para probar</div>
...
<div id="stats-container" class="stats-grid">
        <div class="stat-card" style="background: rgba(31, 111, 235, 0.1); border: 1px solid rgba(31, 111, 235, 0.3);">
            <div style="font-size: 13px; color: #58a6ff; margin-bottom: 5px; font-weight: 500;">Total viajes</div>
        ...
        <div class="stat-card" style="background: rgba(35, 134, 54, 0.1); border: 1px solid rgba(35, 134, 54, 0.3);">
            <div style="font-size: 13px; color: #3fb950; margin-bottom: 5px; font-weight: 500;">Total tiempo de conducción</div>
        ...
        <div class="stat-card" style="background: rgba(227, 76, 38, 0.1); border: 1px solid rgba(227, 76, 38, 0.3);">
            <div style="font-size: 13px; color: #f0883e; margin-bottom: 5px; font-weight: 500;">Voltaje mínimo (motor en frío)</div>
        ...
        <div class="stat-card" style="background: rgba(137, 87, 229, 0.1); border: 1px solid rgba(137, 87, 229, 0.3);">
            <div style="font-size: 13px; color: #a371f7; margin-bottom: 5px; font-weight: 500;">Voltaje medio de conducción (generador)</div>
...
<div id="trip-section" style="margin-top: 20px; padding: 15px; background: #161b22; border-radius: 10px; border: 1px dashed #30363d; display: none;">
        <h3 id="ui-trip-title" style="color: #c9d1d9; font-size: 16px; margin-top: 0; margin-bottom: 15px;">Detectar viajes</h3>
...
(function() {
    ...
    function renderTrips(result) {
        ...
        const modeSuffix = result.mode === 'LiFePO4' ? ' [Modo LiFePO4]' : ' [Plomo ácido / Modo de control de carga]';
        document.getElementById('ui-trip-title').innerText = 'Detectar viajes' + modeSuffix;
    ...
    tChart = new Chart(document.getElementById('tempChart'), createChartConfig('Temperatura (°C)', '#f85149', 'Temperatura (°C)'));
    ...
    function parseExcelData(arrayBuffer) {
        statusText.innerHTML = '⏳ Análisis inteligente de viajes en curso...';
        setTimeout(() => {
            try {
                const workbook = XLSX.read(new Uint8Array(arrayBuffer), { type: 'array' });
                const ws = workbook.Sheets[workbook.SheetNames.find(n => n.includes("Historial") || n.includes("歷史") || n.includes("History")) || workbook.SheetNames[0]];
                ...
                statusText.innerHTML = `✅ Datos analizados correctamente: <b style="color:#58a6ff;">${allData.length}</b> registros.`;
            } catch (error) {
                statusText.innerHTML = `❌ El análisis del documento ha fallado. Por favor, compruebe que el formato del archivo es correcto. (${error.message})`;
            }
        }, 50);
    }
    ...
    loadSampleBtn.onclick = () => {
        statusText.innerHTML = '📥 Descargando archivo de ejemplo...';
        fetch('/files/bm6_2025_10.xls')
            .then(response => {
                if (!response.ok) {
                    throw new Error('El archivo de ejemplo no se ha podido cargar. Por favor, compruebe la ruta.');
                }
            ...
            .catch(error => {
                statusText.innerHTML = `❌ Carga fallida: ${error.message}`;
            });
    };
