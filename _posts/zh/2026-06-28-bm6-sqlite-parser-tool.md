---
layout: post
title:  "BM6/BM200 電池測試資料庫 (SQLite) 解析器"
lang: zh-Hant
date:   2026-06-28 09:30:00
categories: Auto
tags: [汽車電池, 電瓶監控, DIY工具, 數據分析]
description: "專為 BM6/BM200 藍牙電瓶監控器設計的 SQLite 本地資料庫解析器。能讀取 BM6.sqlite 檔案，解析出完整的歷史行駛紀錄、啟動測試波形與充電波紋測試，並提供豐富的縮放圖表與偵測統計數據。"
keywords: "BM6, SQLite解析, 啟動波形, 啟動電壓, 行程偵測, 溫度分析, 充電波紋, 電瓶監控"
image: /images/bm6-sqlite-preview2.webp
faq:
  - question: "這個解析器與月度記錄檔分析工具有何不同？"
    answer: "本工具直接讀取 BM6 APP 在手機本地儲存的完整 SQLite 資料庫檔案。除了能分析出月度歷史電壓與行駛行程外，更可以解析出高頻率（100Hz）的引擎啟動測試電壓波形（Cranking Waveform）與充電/波紋檢測紀錄。"
  - question: "如何取得 BM6.sqlite 檔案？"
    answer: "在 iPhone 或 Android 手機中，您可以透過檔案分享或備份軟體（如 iTunes、iMazing、Android 檔案傳輸等），進入 BM6 APP 的沙盒目錄（Documents），即可找到名為 BM6.sqlite 或 BM6.db 的資料庫檔案。"
  - question: "上傳 sqlite 檔案會有隱私外洩風險嗎？"
    answer: "完全不會。此工具使用 SQL.js 本地 WebAssembly 技術，所有的資料庫載入、查詢與圖表繪製都是在您的瀏覽器沙盒中完成的，沒有 any 資料會被傳送到外部伺服器。"
  - question: "為什麼啟動波形上會有紅點與橘點？"
    answer: "紅點 (Min) 代表啟動馬達拖曳時的最低瞬態電壓；橘點 (Starter) 則是透過動態門檻演算法所判定，啟動馬達電源切開、發電機接手發電的瞬間轉折點。"
---
...
<div id="bm6-sqlite-app-container">
    ...
    <h2 id="ui-title" style="text-align: center; color: #c9d1d9; margin-bottom: 10px; margin-top: 10px;">BM6 電池測試資料庫 (SQLite) 解析器</h2>
    ...
    <div class="upload-section" style="margin-bottom: 30px; padding: 20px; background: #161b22; border-radius: 8px; border: 2px dashed #1f6feb; text-align: center;">
        <p>請選擇您的 <b>BM6.sqlite</b> 檔案（資料完全在您的電腦上處理，不會上傳）：</p>
    ...
    <div id="filter-section" style="display: none; background: #161b22; padding: 20px; border-radius: 8px; margin-bottom: 30px; border: 1px solid #30363d; text-align: center;">
        <div style="display: flex; gap: 20px; flex-wrap: wrap; align-items: center; justify-content: center;">
            <div style="display: flex; align-items: center; gap: 8px;">
                <label style="font-weight: bold; color: #c9d1d9;">選擇設備 (MAC)：</label>
            ...
            <div style="display: flex; align-items: center; gap: 8px;">
                <label style="font-weight: bold; color: #c9d1d9;">選擇月份：</label>
    ...
    <div id="history-section" class="section">
        ...
        <div style="background: rgba(210, 153, 34, 0.1); padding: 15px 20px; border-radius: 10px; margin-bottom: 25px; border-left: 5px solid #d29922; font-size: 14.5px; line-height: 1.6; color: #8b949e;">
            <strong style="color: #d29922;">歷史記錄行程分析</strong><br>
            <span>自動解析歷史電壓與溫度波動，識別行駛行程並統計發電機與電瓶健康度。支援滾輪/雙指縮放與拖曳平移。</span>
        </div>
        ...
        <div style="background: #161b22; border: 1px solid #30363d; padding: 15px 25px; border-radius: 10px; margin-bottom: 25px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 15px;">
            <div style="display: flex; gap: 10px; align-items: center;">
                <label style="font-weight: bold; color: #c9d1d9; font-size: 14px;">顯示範圍：</label>
        ...
        <div style="background: #161b22; padding: 15px; border-radius: 10px; border: 1px solid #30363d; margin-bottom: 25px; display: flex; flex-direction: column; gap: 20px;">
            <div>
                <div class="chart-header-action">
                    <span style="font-size: 14px; font-weight: bold; color: #58a6ff;">⚡ 歷史電壓趨勢 (Voltage)</span>
                    <button class="chart-save-btn" onclick="saveChartWithWatermark('voltageChart', 'bm6_history_voltage.png', 'BM6 歷史電壓趨勢')">
                        📷 截圖存檔
                    </button>
                </div>
            ...
            <div>
                <div class="chart-header-action">
                    <span style="font-size: 14px; font-weight: bold; color: #f85149;">🌡️ 歷史溫度趨勢 (Temperature)</span>
                    <button class="chart-save-btn" onclick="saveChartWithWatermark('tempChart', 'bm6_history_temperature.png', 'BM6 歷史溫度趨勢')">
                        📷 截圖存檔
                    </button>
                </div>
                <div style="height: 280px; width: 100%;">
    ...
    <div id="charging-section" class="section">
        <h2>⚡ 充電與波紋測試紀錄</h2>
        <p>此表紀錄了包含 怠速/高速充電電壓、二極體紋波電壓 等數據。</p>
        <table id="charging-table">
            <thead>
                <tr>
                    <th style="width: 25%;">設備與時間資訊</th>
                    <th style="width: 75%;">解析出的關鍵資料 (與 APP 同步)</th>
                </tr>
            </thead>
            <tbody></tbody>
        </table>
    ...
    <div id="cranking-section" class="section">
        <h2>🚗 啟動測試波形</h2>
        <p>此表紀錄了高頻率的電壓變動數值，已自動為您繪製成圖表。</p>
        <div id="cranking-stats-container" class="stats-grid-2">
            <div class="stat-card" style="background: rgba(31, 111, 235, 0.1); border: 1px solid rgba(31, 111, 235, 0.3);">
                <div style="font-size: 13px; color: #58a6ff; margin-bottom: 5px; font-weight: 500;">平均啟動時間</div>
            ...
            <div class="stat-card" style="background: rgba(227, 76, 38, 0.1); border: 1px solid rgba(227, 76, 38, 0.3);">
                <div style="font-size: 13px; color: #f0883e; margin-bottom: 5px; font-weight: 500;">平均啟動電壓</div>
...
<script>
    ...
    document.getElementById('dbfile').addEventListener('change', function(event) {
        ...
        reader.onload = function() {
            ...
            try {
                ...
                alert("讀取資料庫失敗，請確認檔案是否正確。錯誤訊息：" + err.message);
            }
        };
        reader.readAsArrayBuffer(file);
    });

    // 輕量級 Apple Binary Plist (bplist00) 與 NSKeyedArchiver 解析器
    ...
    function parseChargingTable(db, serial, month) {
        ...
        try {
            ...
            if (res.length === 0) {
                tbody.innerHTML = '<tr><td colspan="2">此設備/月份無充電測試紀錄。</td></tr>';
                return;
            }
            ...
            values.forEach(row => {
                ...
                const mac = row[macIndex] || "未知設備";
                let maskedMac = mac;
                if (mac && mac !== "未知設備") {
                    const cleanMac = String(mac).replace(/[: -]/g, '');
                    maskedMac = cleanMac.length > 3 ? `***${cleanMac.slice(-3)}` : `***${cleanMac}`;
                }
                ...
                let rippleVolStr = "無資料";
                let loadVolStr = "無資料";
                let noLoadVolStr = "無資料";
                ...
                if (blob) {
                    try {
                        ...
                        if (parsedData) {
                            ...
                            // 2. 處理高速充電電壓 (Load)
                            loadVolStr = parsedData.loadVol !== undefined ? `${Number(parsedData.loadVol).toFixed(2)} V` : "無資料";
                            
                            // 3. 處理怠速充電電壓 (NoLoad)
                            noLoadVolStr = parsedData.noLoadVol !== undefined ? `${Number(parsedData.noLoadVol).toFixed(2)} V` : "無資料";
                            
                            parseSuccess = true;
                        }
                    } catch (err) {
                        console.error("解析 bplist 失敗:", err);
                    }
                }

                tr.innerHTML = `
                    <td>
                        <b>MAC:</b> ${maskedMac} <br>
                        <b>時間:</b> ${date}
                    </td>
                    <td>
                        <b>怠速充電電壓 (NoLoad):</b> <span style="color:#58a6ff; font-weight:bold;">${noLoadVolStr}</span> <br>
                        <b>高速充電電壓 (Load):</b> <span style="color:#3fb950; font-weight:bold;">${loadVolStr}</span> <br>
                        <b>二極體紋波電壓 (Ripple):</b> <span style="color:#d2a8ff; font-weight:bold;">${rippleVolStr}</span> <br>
                        <small style="color:gray;">解析狀態: ${parseSuccess ? "🟢 成功精準解析" : "🔴 解析失敗"}</small>
                    </td>
                `;
                tbody.appendChild(tr);
            });
        } catch (e) {
            tbody.innerHTML = `<tr><td colspan="2">解析錯誤: ${e.message}</td></tr>`;
        }
    }
    ...
    function parseCrankingTable(db, serial, month) {
        ...
        try {
            ...
            const res = db.exec(sql);
            if (res.length === 0) {
                container.innerHTML = '<p>此設備/月份無啟動波形紀錄。</p>';
                ...
                if (extractedNumbers.length > 0) {
                    ...
                    if (serialIndex !== -1 && row[serialIndex] && ts) {
                        ...
                        try {
                            ...
                            console.error("查詢歷史溫度失敗:", tempErr);
                        }
                    }

                    let titleText = `啟動時間: ${crankingDuration.toFixed(2)}秒 - `;
                    if (dateStr) {
                    ...
                    if (eventTemp !== null && eventTemp !== undefined) {
                        titleText += ` (溫度: ${eventTemp}°C)`;
                    } else if (tempIndex !== -1 && row[tempIndex] !== undefined) {
                        titleText += ` (溫度: ${row[tempIndex]}°C)`;
                    }
                    titleText += ` (初始: ${vInitial.toFixed(2)}V)`;
                    ...
                    saveBtn.innerHTML = '📷 截圖存檔';
                    saveBtn.title = '下載帶有浮水印的圖表截圖';
                    ...
                    wrapper.innerHTML = `<h3>測試紀錄 #${row[0] || index + 1}</h3><p style="color:red; margin: 15px 0;">無法從這筆紀錄中解析出連續的電壓數值。</p><hr>`;
            ...
            container.innerHTML = `<p>讀取失敗: ${e.message}</p>`;
            document.getElementById('cranking-stats-container').style.display = 'none';
        }
    }
    ...
    function parseHistoryTable(db, serial, month) {
        ...
        try {
            ...
            if (res.length === 0) {
                ...
                document.getElementById('dateSelector').innerHTML = '<option value="all">-- 無日期 --</option>';
                return;
            }
    ...
    function initMonthSelector(db, serial) {
        ...
        monthSelector.innerHTML = '<option value="all">-- 全部月份 --</option>';
    ...
    function saveChartWithWatermark(chartOrCanvas, defaultFileName, chartTitle) {
        ...
        if (chartTitle) {
            ...
            ctx.fillStyle = '#c9d1d9';
            ctx.font = `bold ${Math.round(14 * ratio)}px "Segoe UI", "Microsoft JhengHei", sans-serif`;
        ...
        ctx.font = `${Math.round(12 * ratio)}px "Segoe UI", "Microsoft JhengHei", sans-serif`;
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ...
        ctx.font = `bold ${Math.round(13 * ratio)}px "Segoe UI", "Microsoft JhengHei", sans-serif`;
        ctx.textAlign = 'right';
        ctx.textBaseline = 'middle';
        ...
        try {
            ...
            console.error("截圖存檔失敗:", err);
            alert("截圖存檔失敗：" + err.message);
        }
    }
