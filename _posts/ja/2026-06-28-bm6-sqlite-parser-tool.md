        ...
        <div style="background: #161b22; padding: 15px; border-radius: 10px; border: 1px solid #30363d; margin-bottom: 25px; display: flex; flex-direction: column; gap: 20px;">
            <div>
                <div class="chart-header-action">
                    <span style="font-size: 14px; font-weight: bold; color: #58a6ff;">⚡ 過去の電圧推移 (Voltage)</span>
                    <button class="chart-save-btn" onclick="saveChartWithWatermark('voltageChart', 'bm6_history_voltage.png', 'BM6 過去の電圧推移')">
                        📷 スクリーンショット保存
                    </button>
                </div>
            ...
            <div>
                <div class="chart-header-action">
                    <span style="font-size: 14px; font-weight: bold; color: #f85149;">🌡️ 過去の温度推移 (Temperature)</span>
                    <button class="chart-save-btn" onclick="saveChartWithWatermark('tempChart', 'bm6_history_temperature.png', 'BM6 過去の温度推移')">
                        📷 スクリーンショット保存
                    </button>
                </div>
                <div style="height: 280px; width: 100%;">
...
<script>
        ...
        try {
            ...
            values.forEach((row, index) => {
                ...
                if (extractedNumbers.length > 0) {
                    ...
                    let titleText = `始動時間: ${crankingDuration.toFixed(2)}秒 - `;
                    if (dateStr) {
                    ...
                    saveBtn.innerHTML = '📷 スクリーンショット保存';
                    saveBtn.title = '透かし付きグラフのスクリーンショットをダウンロード';
    ...
    function saveChartWithWatermark(chartOrCanvas, defaultFileName, chartTitle) {
        ...
        if (chartTitle) {
            ...
            ctx.fillStyle = '#c9d1d9';
            ctx.font = `bold ${Math.round(14 * ratio)}px "Segoe UI", "Meiryo", sans-serif`;
        ...
        ctx.font = `${Math.round(12 * ratio)}px "Segoe UI", "Meiryo", sans-serif`;
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ctx.fillText('BM6 バッテリーモニター', Math.round(16 * ratio), footerY + watermarkHeight / 2);
        
        // フッター右側 (透かしURL)
        ctx.fillStyle = '#8b949e';
        ctx.font = `bold ${Math.round(13 * ratio)}px "Segoe UI", "Meiryo", sans-serif`;
        ctx.textAlign = 'right';
        ctx.textBaseline = 'middle';
        ...
        try {
            ...
            alert("スクリーンショットの保存に失敗しました: " + err.message);
        }
    }
